odoo.define('uppercrust_backend_theme.DashboardCustomizeTheme', function (require) {
    ...
    var CustomizeThemeDialog = Dialog.extend({
        ...
        _fetchThemeData: function (theme_id) {
            ...
            new Model('ir.web.theme').call('search_read', [[['id', '=', theme_id]]], {'fields': form_fields}).then(function (result) {
                self._setSelectedTheme(result[0]);
            });
        },
        _removeTheme: function ($li, res_id) {
            var self = this;
            new Model('ir.web.theme').call('unlink', [parseInt(res_id, 10)]).then(function (value) {
                $li.remove();
        ...
        _createRecord: function (form_values) {
            return new Model('ir.web.theme').call('create', [form_values], {context: session.user_context});
        },
        _updateRecord: function (theme_id, form_values) {
            return new Model('ir.web.theme').call('write', [[theme_id], form_values], {});
        },
        _changeCurruntTheme: function (theme_id) {
            var self = this;
            return new Model('ir.web.theme').call('set_customize_theme', [[], theme_id, self.form_values]).then(function () {
                location.reload();
            });
        },
    ...
    var DashboardCustomizeTheme = Widget.extend({
        ...
        _checkStatusbar: function () {
            var self = this;
            return new Model('ir.config_parameter').call('get_param', ['web_status_tags.is_status_tags']).then(function (is_status_tags) {
        ...
        _onClickSetupTheme: function () {
            ...
            self._checkStatusbar().then(function (is_status_tags) {
                ...
                new Model('ir.web.theme').call('search_read', [[]], {'fields': form_fields}).then(function (result) {
                    new Model('ir.config_parameter').call('get_param', ['uppercrust_backend_theme.selected_theme']).then(function (theme_id) {
