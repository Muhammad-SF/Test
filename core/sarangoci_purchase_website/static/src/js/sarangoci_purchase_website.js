odoo.define('sarangoci_purchase_website.cart_sale', function (require) {
//     "use strict";
    ...
    var sale_cart_link = $('ul#top_menu li a[href$="/sale/cart"]');
    var sale_cart_link_counter;
    sale_cart_link.popover({
        trigger: 'manual',
        animation: true,
        html: true,
        title: function () {
        ...
        var self = this;
        clearTimeout(sale_cart_link_counter);
        sale_cart_link.not(self).popover('hide');
        sale_cart_link_counter = setTimeout(function(){
            if($(self).is(':hover') && !$(".mycart-popover:visible").length)
            {
                $.get("/sale/cart", {'type': 'popover'})
                    .then(function (data) {
...
odoo.define('sarangoci_purchase_website.sarangoci_purchase_website', function (require) {
    "use strict";
    ...
    $('.oe_website_sale').each(function () {
        ...
        $(oe_website_sale).on("change", 'input[name="add_qty"]', function (event) {
            ...
            if ($("#product_detail").length) {
                ...
                ajax.jsonRpc("/sale/get_unit_price", 'call', {'product_ids': product_ids,'add_qty': parseInt(qty)})
                .then(function (data) {
                    _.each(product_dom, function (prod) {
        ...
        $(oe_website_sale).on("change", ".oe_cart input.js_quantity[data-product-id]", function () {
          ...
          clickwatch(function(){
            ...
            ajax.jsonRpc("/sale/cart/update_json", 'call', {
                ...
                $(".js_cart_lines").first().before(data['sarangoci_purchase_website.cart_lines']).end().remove();
        ...
        $('.oe_cart').on('click', '.js_edit_address', function() {
            $(this).parent('div.one_kanban').find('form.hide').attr('action', '/sale/address').submit();
        });
