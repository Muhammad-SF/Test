...
#                        emp_leave_ids = rec.employee_id.leave_config_id.holiday_group_config_line_ids.ids
    ...
    def _check_holiday_to_from_dates(self, start_date, end_date, employee_id):
        ...
        for remov in remove_date:
            if remov in dates:
                dates.remove(remov)
        date_f = datetime.datetime.strptime(start_date,DEFAULT_SERVER_DATETIME_FORMAT).date()
        ...
        no_of_day = 0.0
        start_date1 = datetime.datetime.strptime(start_date,DEFAULT_SERVER_DATETIME_FORMAT).date().strftime(DEFAULT_SERVER_DATE_FORMAT)
        end_date1 = datetime.datetime.strptime(end_date,DEFAULT_SERVER_DATETIME_FORMAT).date().strftime(DEFAULT_SERVER_DATE_FORMAT)
        for day in dates:
    ...
    @api.multi
    def _check_holiday_to_from_dates1(self, start_date, end_date, employee_id):
        '''
        Checks that there is a public holiday,Saturday and Sunday on date of leave
        @param self : Object Pointer
        @param cr : Database Cursor
    ...
    def create(self, vals):
        ...
        type_add = False
        type_remove = False
        least_date = {'number_of_days_temp': 0.0}
        if 'type' in vals and vals['type'] =='add':
            type_add = True
        if 'date_from' in vals and 'date_to' not in vals and 'type' in vals and vals['type'] =='remove':
            least_date = self.onchange_date_from(date_from=vals['date_from'], date_to=vals['date_to'],half_day=False,holiday_status_id=vals['holiday_status_id'],employee_id=vals['employee_id'])
        elif 'date_from' not in vals and 'date_to' in vals and 'type' in vals and vals['type'] =='remove':
        ...
        elif 'date_to' in vals and 'date_from' in vals and 'type' in vals and vals['type'] == 'remove':
            least_date = self.onchange_date_to(date_from=vals['date_from'], date_to=vals['date_to'], half_day=False,
        ...
        if 'half_day' in vals and vals['half_day'] ==True and 'type' in vals and vals['type'] =='remove':
            if 'date_from' in vals:
                date_to_with_delta = datetime.datetime.strptime(vals['date_from'], DEFAULT_SERVER_DATE_FORMAT) + datetime.timedelta(hours=4)
            ...
            if 'half_day' not in vals and rec.half_day == False:
                ...
                if vals['half_day'] ==True:
                    if 'date_from' in vals :
                        ...
                        date_to_with_delta = datetime.datetime.strptime(rec.date_from, DEFAULT_SERVER_DATE_FORMAT) + datetime.timedelta(hours=4)
                        vals.update({'date_to': date_to_with_delta,'number_of_days_temp':0.50})
...
#                        vals.update({'number_of_days_temp':least_date['value']['number_of_days_temp']})
    ...
    def _get_number_of_daystmp(self, date_from, date_to):
        ...
        from_dt = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATE_FORMAT)
        to_dt = datetime.datetime.strptime(date_to, DEFAULT_SERVER_DATE_FORMAT)
    ...
    def onchange_date_from(self,date_from=False, date_to=False,half_day=False, holiday_status_id=False, employee_id=False):
        ...
        for rec in self:
            ...
            if date_from and not date_to:
                date_to_with_delta = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATE_FORMAT) + datetime.timedelta(hours=8)
                result['value']['date_to'] = str(date_to_with_delta)
            ...
            if date_from and date_to and half_day == True:
                date_to_with_delta = datetime.datetime.strptime(date_from, DEFAULT_SERVER_DATE_FORMAT) + datetime.timedelta(hours=4)
                result['value']['date_to'] = str(date_to_with_delta)
            if half_day == True:
    ...
    def onchange_half_day(self):
        ...
        if self.half_day == True:
            if self.date_from != False:
                date_to_with_delta = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATE_FORMAT) + datetime.timedelta(hours=4)
            ...
            if self.date_from:
                df = datetime.datetime.strptime(self.date_from, DEFAULT_SERVER_DATE_FORMAT) + relativedelta(hours=8)
                self.date_to = df
...
#                    joining_date = datetime.datetime.strptime(employee.join_date, DEFAULT_SERVER_DATE_FORMAT).date()
