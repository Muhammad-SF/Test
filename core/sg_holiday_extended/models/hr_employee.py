...
#                    emp_leave_ids.append(leave.leave_type_id.id)
                if employee.leave_config_id.holiday_group_config_line_ids:
                    for holiday in employee.leave_config_id.holiday_group_config_line_ids:
                        if  holiday.leave_type_id.pro_rate:
                            leave_count = 0
                            employee_join_date = datetime.datetime.strptime(employee.join_date, DEFAULT_SERVER_DATE_FORMAT)
                            year_end_date = datetime.datetime.strptime(str(date_today.year) + '-12-31', DEFAULT_SERVER_DATE_FORMAT)
                            r = relativedelta.relativedelta(year_end_date, employee_join_date)
                            leave_count += r.months
                            if int(employee_join_date.day) <= holiday.leave_type_id.cut_off_date:
                                leave_count += 1
                            if leave_count > 0:
                                if holiday.leave_type_id.gender_leave and employee.gender != holiday.leave_type_id.gender_leave:
                                    leave_dict = {
                                        'name': 'Assign Default ' + str(holiday.leave_type_id.name2),
                                        'employee_id': employee.id,
                                        'holiday_type': 'employee',
                                        'holiday_status_id': holiday.leave_type_id.id,
                                        'number_of_days_temp': leave_count,
                                        'hr_year_id': curr_hr_year_id,
                                        'state': 'refuse',
                                        'check_employee_warning': False,
                                    }
                                else:
                                    leave_dict = {
                                        'name': 'Assign Default ' + str(holiday.leave_type_id.name2),
...
class dependents(models.Model):
    ...
    relation_ship = fields.Selection([('son', 'Son'), ('daughter', 'Daughter'), ('father', 'Father') , ('mother', 'Mother'), ('husband', 'Husband') , ('wife', 'Wife')], string='Relationship')
