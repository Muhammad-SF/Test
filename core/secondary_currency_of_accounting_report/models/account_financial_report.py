...
class ReportAccountFinancialReport(models.Model):
    ...
    def get_lines(self, context_id, line_id=None):
        ...
        period_from = context_id.date_from and context_id.date_from or ''
        period_to = context_id.date_to and context_id.date_to or ''
        IrModel = self.env['ir.model']
        fiscalyear_search = IrModel.search([('model', '=', 'account.fiscalyear')], limit=1)
        if context_id.date_to and fiscalyear_search:
            period = self.env['account.fiscalyear'].search(
                [('date_start', '<=', context_id.date_to), ('date_stop', '>=', context_id.date_to)], limit=1)
            if period:
                period_from = period.date_start
                period_to = period.date_stop
...
class AccountFinancialReportLine(models.Model):
    ...
    def _format(self, value):
        ...
        if self.figure_type == 'float':
            currency_id = self.env.context.get('change_currency') and self.env.context.get('change_currency') or self.env.user.company_id.currency_id
    ...
    def _compute_line(self, currency_table, financial_report, group_by=None, domain=[]):
        ...
        if context.get('change_currency'):
            Balance = []
            Credit = []
            Debit = []
            AmountResidual = []
            ...
            AmlSearch = aml_obj.search(domain)
            for Aml in AmlSearch:
                currency_id = context.get('change_currency').with_context(date=Aml.date)
                ...
                AmountResidual.append(Aml.amount_residual and  UsedCurrency.compute(Aml.amount_residual, currency_id) or 0.0)
        ...
        if (context.get('sum_if_pos') or context.get('sum_if_neg')) and group_by:
            ...
            for row in self.env.cr.dictfetchall():
                if (row['balance'] > 0 and context.get('sum_if_pos')) or (row['balance'] < 0 and context.get('sum_if_neg')):
        ...
        if context.get('change_currency'):
            results = {'credit': sum(Credit) and sum(Credit) or 0.0, 'amount_residual': sum(AmountResidual) and sum(AmountResidual) or 0.0, 'balance': sum(Balance) and sum(Balance) or 0.0, 'debit': sum(Debit) and sum(Debit) or 0.0}
        results['currency_id'] = context.get('change_currency') and context.get('change_currency') or  self.env.user.company_id.currency_id.id
        return results
            ...
            if self.env.context.get('change_currency'):
                ...
                if self.type == 'account':
                    for account in self.account_ids:
                        AmlSearch = aml_obj.search(domain + [('account_id', '=', account.id)])
                        Balance = []
                        AmountResidual = []
                        Debit = []
                        Credit = []
                        for Aml in AmlSearch:
                            ...
                            Balance.append(Aml.balance and UsedCurrency.compute(Aml.balance,
                                                                               currency_id) or 0.0)
                            AmountResidual.append(Aml.amount_residual and UsedCurrency.compute(Aml.amount_residual,
                                                                                       currency_id) or 0.0)
                            Debit.append(Aml.debit and UsedCurrency.compute(Aml.debit,
                                                                           currency_id) or 0.0)
                            Credit.append(Aml.credit and UsedCurrency.compute(Aml.credit,
                                                                             currency_id) or 0.0)
                        if AmountResidual or Balance or Debit or Credit:
                            results.append((account.id, sum(Balance), sum(AmountResidual), sum(Debit), sum(Credit)))
                elif self.type == 'account_type':
                    AccountSearch = self.env['account.account'].search(
                        [('user_type_id', 'in', self.account_type_ids.ids)])
                    for account in AccountSearch:
                        AmlSearch = aml_obj.search(domain + [('account_id', '=', account.id)])
                        Balance = []
                        AmountResidual = []
                        Debit = []
                        ...
                        for Aml in AmlSearch:
                            ...
                            Credit.append(Aml.credit and UsedCurrency.compute(Aml.credit,
                                                                             currency_id) or 0.0)
                        if sum(AmountResidual) or sum(Balance) or sum(Debit) or sum(Credit):
                            results.append((account.id, sum(Balance), sum(AmountResidual), sum(Debit), sum(Credit)))
                ...
                TotalDebit = []
                for line in results:
                    TotalBal.append(line[1])
                    TotalCredit.append(line[3])
                    TotalDebit.append(line[4])
                vals['balance'] = sum(TotalBal) and sum(TotalBal) or 0.0
                if debit_credit:
                    vals['credit'] = sum(TotalCredit) and sum(TotalCredit) or 0.0
                    vals['debit'] = sum(TotalDebit) and sum(TotalDebit) or 0.0

            results = dict(
                [(k[0], {'balance': k[1], 'amount_residual': k[2], 'debit': k[3], 'credit': k[4]}) for k in results])

            c = FormulaContext(self.env['account.financial.html.report.line'], linesDict, currency_table,
                               financial_report, only_sum=True)
            if formulas:
                for key in results:
        ...
        for line in self:
            ...
            for period in comparison_table:
                ...
                if line.special_date_changer == 'strict_range':
                    strict_range = True
                if line.special_date_changer == 'active_fiscal_year':
                    # config_setting = self.env['account.config.settings'].search([], order='id desc', limit=1)
                    # period_from = config_setting.fiscalyear.date_start
                    # period_to = config_setting.fiscalyear.date_stop
                    period_from = line._context.get('period_from')
                    period_to = line._context.get('period_to')
            ...
            if line in context.unfolded_lines or line.show_domain == 'always':
                ...
                for domain_id in domain_ids:
                    ...
                    vals = {
                        ...
                        'columns': [i for i in res[domain_id]],
                    }
                ...
                if domain_ids:
                    lines.append({
                        ...
                        'columns': [i for i in list(lines[0]['columns'])],
                    })

            for vals in lines:
                # Apply normal_balance calculation
                # if line.normal_balance == 'credit' and not context.analytic_level_id:
                if line.normal_balance == 'credit':
                    vals['columns'] = [value and -value for value in vals['columns']]
                if len(comparison_table) == 2:
                    ...
                    vals['columns'] = map(line._format, [i for i in vals['columns']])
    ...
    def __getitem__(self, item):
        ...
        if line_id:
            ...
            if line_id.special_date_changer == 'to_beginning_of_period' and line_id._context.get('date_from'):
                ...
                period_to = date_tmp.strftime('%Y-%m-%d')
                period_from = False
            if line_id.special_date_changer == 'active_fiscal_year':
                period_from = line_id._context.get('period_from')
                period_to = line_id._context.get('period_to')
...
class AccountFinancialReportContext(models.TransientModel):
    ...
    fold_model = 'account.financial.html.report.line'
