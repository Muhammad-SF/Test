...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def do_query(self, line_id):
        ...
        if self.env.context.get('change_currency'):
            ...
            if self.env.context['account_types']:
                domain.append(('account_id.internal_type', '=', self.env.context['account_types']))

            if line_id:
                domain.append(('partner_id', '=', line_id))

            AmlSearch = AmlObj.search(domain)
            partner_ids = set([aml.partner_id.id for aml in AmlSearch])


            for Partner in partner_ids:
                AmlSearch = AmlObj.search(domain + [('partner_id', '=', Partner)])
                Balance = []
                AmountCurrency = []
                Debit = []
                Credit = []
                for Aml in AmlSearch:
                    ...
                    Balance.append(Aml.balance and UsedCurrency.compute((Aml.debit - Aml.credit),
                                                                       currency_id) or 0.0)
                    AmountCurrency.append(Aml.amount_currency and UsedCurrency.compute(Aml.amount_currency,
                                                                                      currency_id) or 0.0)
                ...
                if AmountCurrency or Balance or Debit or Credit:
                    results.append((Partner, sum(Balance), sum(AmountCurrency), sum(Debit), sum(Credit)))
    ...
    def _lines(self, line_id=None):
        ...
        grouped_partners = self.with_context(date_from_aml=context['date_from'], date_from=context['date_from'] and company_id.compute_fiscalyear_dates(datetime.strptime(context['date_from'], DEFAULT_SERVER_DATE_FORMAT))['date_from'] or None).group_by_partner_id(line_id)
        ...
        for partner in sorted_partners:
            ...
            lines.append({
                ...
                'columns': [self._format(debit), self._format(credit), self._format(abs(balance))],
                'level': 2,
                'unfoldable': True,
                'unfolded': partner in unfold_partners,
                'colspan': 5,
            })

            if partner in unfold_partners:
                progress = 0
                domain_lines = []
                ...
                domain_lines[:0] = [{
                    ...
                    'columns': ['', '', '', '', self._format(initial_debit), self._format(initial_credit), self._format(abs(initial_balance))],
                    'level': 1,
                }]
                domain_lines.append({
                    'id': partner.id,
