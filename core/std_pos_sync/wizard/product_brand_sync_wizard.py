...
class ProductBrandSyncWizard(models.TransientModel):
    _name = 'product.brand.sync.wizard'
    
    
    @api.multi
    def action_product_brand_sync(self):
        ...
        product_brand_pool = self.env['product.brand']
        synclog_pool = self.env['product.brand.sync.log']
        model_id = self.env['ir.model'].search([('model', '=', 'product.brand')])
        
        con_model_id = sock.execute(database_name, uid, password, 'ir.model','search', [('model', '=', 'product.brand')])
        latest_auditlogs = []
        ...
        for log_id in latest_auditlogs:
            ...
            pos_sync_id = sock.execute(database_name, uid, password, 'product.brand','read',log_data[0]['res_id'],['pos_sync_id'])
            if pos_sync_id:
                product_brand_obj = product_brand_pool.search([('pos_sync_id','=', pos_sync_id[0]['pos_sync_id'])])
                ...
                if log_data[0]['method'] == 'write':
                    if product_brand_obj:
                        product_brand_obj.write(vals)
                if log_data[0]['method'] == 'create':
                    if not product_brand_obj:
                        product_brand_id = product_brand_pool.create(vals)
