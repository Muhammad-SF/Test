...
class PaymentTermSyncWizard(models.TransientModel):
    _name = 'payment.term.sync.wizard'
    
    
    @api.multi
    def action_payment_term_sync(self):
        ...
        synclog_pool = self.env['account.journal.sync.log']
        model_id = self.env['ir.model'].search([('model', '=', 'account.journal')])
        
        con_model_id = sock.execute(database_name, uid, password, 'ir.model','search', [('model', '=', 'account.journal')])
        latest_auditlogs = []
        ...
        for log_id in latest_auditlogs:
            ...
            pos_sync_id = sock.execute(database_name, uid, password, 'account.journal','read',log_data[0]['res_id'],['pos_sync_id'])
            if pos_sync_id:
                journal_obj = journal_pool.search([('pos_sync_id','=', pos_sync_id[0]['pos_sync_id'])])
                ...
                if log_data[0]['method'] == 'write':
                    if journal_obj:
                        journal_obj.write(vals)
                if log_data[0]['method'] == 'create':
                    if vals.has_key('journal_user') and vals.get('journal_user'):
                        journal_id = journal_pool.create(vals)
