...
class CustomerSyncWizard(models.TransientModel):
    _name = 'customer.sync.wizard'
    
    
    @api.multi
    def action_customer_sync(self):
        ...
        synclog_pool = self.env['customer.sync.log']
        model_id = self.env['ir.model'].search([('model', '=', 'res.partner')])
        
        con_model_id = sock.execute(database_name, uid, password, 'ir.model','search', [('model', '=', 'res.partner')])
        latest_auditlogs = []
        ...
        for log_id in latest_auditlogs:
            ...
            pos_sync_id = sock.execute(database_name, uid, password, 'res.partner','read',log_data[0]['res_id'],['pos_sync_id'])
            if pos_sync_id:
                partner_obj = partner_pool.search([('pos_sync_id','=', pos_sync_id[0]['pos_sync_id'])])
    ...
    @api.multi
    def action_customer_sync_to_master(self):
        master_url = self.env.ref('std_pos_sync.master_url').value
        database_name = self.env.ref('std_pos_sync.database_name').value
        company_name = self.env.ref('std_pos_sync.company_name').value
        username = self.env.ref('std_pos_sync.username').value
        ...
        synclog_pool = self.env['master.customer.sync.log']
        auditlog_pool = self.env['auditlog.log']
        partner_pool = self.env['res.partner']
        con_model_id = self.env['ir.model'].search([('model', '=', 'res.partner')],limit=1)
        ...
        for log_id in latest_auditlogs:
            partner_obj = partner_pool.browse(log_id.res_id)
            if partner_obj:
                ...
                if log_id.method == 'write':
                    if master_partner_id:
                        sock.execute(database_name, uid, password, 'res.partner','write',master_partner_id[0],vals)
                if log_id.method == 'create':
                    if not master_partner_id:
                        sock.execute(database_name, uid, password, 'res.partner','create',vals)
