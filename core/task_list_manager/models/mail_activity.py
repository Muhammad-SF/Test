...
class MailActivity(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not fields or 'res_model_id' in fields and res.get('res_model'):
            res['res_model_id'] = self.env['ir.model'].search([('model','=',res.get('res_model'))])[0].id
        return res
    ...
    def action_feedback(self, feedback=False):
        ...
        for activity in self:
            ...
            record.message_post_with_view(
                'task_list_manager.message_activity_done',
                values={'activity': activity},
                subtype_id=self.env.ref('task_list_manager.mt_activities').id,
...
class ResUsers(models.Model):
    ...
    def activity_user_count(self):
        query = """SELECT act.res_id,m.name, count(*), act.res_model as model,
                        CASE
        ...
        for activity in activity_data:
            ...
            if other_data.get(activity['model']):
                other_data[activity['model']].append(activity['res_id'])
            else:
                other_data[activity['model']] = [activity['res_id']]
        for model,model_value in user_activities.iteritems():
            for key,o_act in other_data.iteritems():
                if key==model:
                    user_activities[model]['ids']=o_act
...
class ProductTemplate(models.Model):
    ...
    _inherit = ['product.template', 'mail.activity.mixin']
...
class AccountVoucher(models.Model):
    ...
    _inherit = ['account.voucher', 'mail.activity.mixin']
