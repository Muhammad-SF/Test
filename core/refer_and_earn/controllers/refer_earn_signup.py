	...
	def web_auth_signup(self, *args, **kw):
		...
		if result.qcontext.get('error'):
			...
			if request.httprequest.method == 'POST':
				if kw.get('referral'):
					checkparentReferral = user.partner_id._createReferralAccount(partner_id=user.partner_id.id,
					...
					if checkparentReferral['is_parent_referral']:
						request.env['transaction.history'].sudo()._createTransactionSignup(partner_id = user.partner_id.id,
																							user_id = user.id,
																							parent_user_id = checkparentReferral['parent_user_id'])
					else:
						request.env['transaction.history'].sudo()._createInvalidTransactionSignup(user_id = user.id,
																							invalid_referal=kw.get('referral'))

				else:
					ReferralCustomer = user.partner_id._createReferralAccount(partner_id=user.partner_id.id,
...
class ReferEarnWebsiteSale(WebsiteSale):
	...
	def payment_confirmation(self, **post):
		...
		user = request.env.user
		checkComsnType = request.env['referral.config.setting'].sudo().get_default_commissionOn().get('verify_commission') == 'afterOrder'
	...
	def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
		...
		if last_saleOrder.use_re_epoints and user.partner_id.referral_earning > 0.0:
			request.env['transaction.history'].sudo()._create_ReferaldebitTransaction(user_id=user.id , 
