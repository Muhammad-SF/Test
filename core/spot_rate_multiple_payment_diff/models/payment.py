...
class AccountPayment(models.Model):
    ...
    def _create_payment_entry(self, amount):
        ...
        # For spot rate register payment
        ...
        if (self.payment_difference_handling == 'reconcile' and self.payment_difference) or (
                self._context.get('payment_difference_handling') == 'reconcile' and self.payment_difference):
            writeoff_line = self._get_shared_move_line_vals(0, 0, 0, move.id, False)
            if self.difference_ids:
                for de in self.difference_ids:
                    writeoff_line = self._get_shared_move_line_vals(0, 0, 0, move.id, False)
                    amount_currency_wo, currency_id = aml_obj.with_context(
                        date=self.payment_date).compute_amount_fields(de.amount, self.currency_id,
                    ...
                    if self.spot_rate:
                        amount_wo = self.spot_rate * amount_currency_wo
                    else:
                        if de.currency_id and de.currency_id.rate:
                            amount_wo = de.amount / de.currency_id.rate
                        else:
                            amount_wo = de.amount
                ...
                # amount in the company currency
                if self.spot_rate:
                    amount_wo = self.spot_rate * self.amount_currency_wo
                if amount_wo > 0:
                    debit_wo = amount_wo
                    credit_wo = 0.0
                    amount_currency_wo = abs(amount_currency_wo)
