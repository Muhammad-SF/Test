...
class AccountPayment(models.Model):
    ...
    def compute_spot_rate_check(self):
        for record in self:
            record.spot_rate_check = False
            for invoice in record.invoice_ids:
                if invoice.currency_id.id != self.env.user.company_id.currency_id.id:
...
class ReceiptPayment(models.Model):
    ...
    def compute_spot_rate_check(self):
        for record in self:
            record.spot_rate_check = False
            if record.currency_id.id != self.env.user.company_id.currency_id.id:
                record.spot_rate_check = True
    ...
    def create_payments(self, invoice_id, payment_method_id, payment_type, amount):
        payment_id = super(ReceiptPayment, self).create_payments(invoice_id, payment_method_id, payment_type, amount)
        payment_id.spot_rate = self.spot_rate
        return payment_id
