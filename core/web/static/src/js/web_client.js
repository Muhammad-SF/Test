...
var SystrayMenuDebug = require('web.SystrayMenuDebug');
...
return AbstractWebClient.extend({
    ...
    show_application: function() {
        ...
        this.systray_menu_debug = new SystrayMenuDebug(this);
        this.systray_menu_debug.setElement(this.$el.parents().find('.oe_systray_debug'));
        var systray_menu_debug_loaded = this.systray_menu_debug.start();

        // Start the menu once both systray and user menus are rendered
        // to prevent overflows while loading
        return $.when(systray_menu_loaded, user_menu_loaded, systray_menu_debug_loaded).then(function() {
            self.menu.start();
