...
class acc_report_resigned_emp(models.TransientModel):
    ...
    company_id = fields.Many2one('res.company', 'Company Name',
                                default = lambda self: self.env['res.company']._company_default_get('acc.report.resigned.emp'))

    @api.multi
    def print_report(self):
        emps = self.env['hr.employee'].search([('accommodated', '=', True),
                                               ('last_date', '>=', self.start_date),
                                               ('last_date', '<=', self.end_date)], order = 'last_date')
        month_list = []
        check_months = []
        for emp in emps:
            ctr = 1
        ...
        module_rec = self.env['acc.report.resigned.emp.standard.export'].create({'file' : buf, 'name' : 'Resigned Employee.xls'})
        return {
