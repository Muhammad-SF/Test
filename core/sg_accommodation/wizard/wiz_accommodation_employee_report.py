...
class wiz_accommodation_employee_report(models.TransientModel):
    ...
    def print_report(self):
        ...
        for comp in company_ids:
            companies.update(dict([(comp.id, comp.company_code)]))
        new_lst = []
            ...
            for room in accom.room_ids:
                ...
                for visa in room.visa_quota_ids:
                    ...
                    for company_id in company_ids:
                        ...
                        bed_ids_filled = self.env['beds.beds'].search([('room_id', '=', room.id),
                                                                       ('employee_id', '!=', False),
                                                                       ('employee_id.company_id', '=', company_id.id),
                                                                       ('employee_id.emp_country_id', '=', visa_nat_id)
                                                                       ])
                        country_comp_total += len(bed_ids_filled.ids)
                        company_dict[companies.get(company_id.id)] = len(bed_ids_filled.ids)
                ...
                new_dict = {
                    ...
                    'acc_name' : room.name,
                    'landlord':'',
					'tenant':accom.paying_comp_id.company_code,
        ...
        ctx = dict(context)
        ctx.update({'file': buf})
        module_rec = self.env['acco.location.report'].create({'name': 'Locationwise.xls', 'file' : buf})
        return {
