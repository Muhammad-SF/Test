    ...
    def check_report(self):
        ...
        for country in wiz_rec.nationality_ids:
            ...
            for visa in visa_ids:
                ...
                room_ids = self.env['room.room'].search([('accommodation_id', '=', visa.accommodation_id.id)])
                for room in room_ids:
                    occupied_beds = self.env['beds.beds'].search([('employee_id.emp_country_id', '=', visa.nationality_id.id),
                            ('employee_id', '!=', False),
                            ('room_id', '=', room.id)])
                    occu_beds = []
                    counter = 0
        ...
        ctx = dict(context)
        ctx.update({'file': buf})
        nationality_rec = self.env['acco.nationality.report'].create({'file': buf, 'name': 'Nationalitywise.xls'})
        return {
                'type': 'ir.actions.act_window',
                'res_id' : nationality_rec.id,
                'view_type': 'form',
                'view_mode': 'form',
