...
class procurement_order(models.Model):
    ...
    def _get_stock_move_values(self):
        ...
        return {
            ...
            'description': self.sale_line_id.name if self.sale_line_id.name else ''
        }
...
class SaleOrderLine(models.Model):
    ...
    @api.multi
    def write(self, vals):
        if vals.get('name', '') and self.state not in ['done', 'cancel']:
            procurement_ids = self.env['procurement.order'].search([('product_id', '=', self.product_id.id), ('sale_line_id', '=', self.id)], limit=1)
            move_ids = self.env['stock.move'].search([('product_id', '=', self.product_id.id), ('procurement_id','=', procurement_ids.id)], limit=1)
            pack_ids = self.env['stock.pack.operation'].search(
                [('product_id', '=', move_ids.product_id.id), ('picking_id', '=', move_ids.picking_id.id)], limit=1)
            if move_ids:
                move_ids.description = vals.get('name')
            if pack_ids:
                pack_ids.description = vals.get('name')
...
class stock_picking(models.Model):
    ...
    def _prepare_pack_ops(self, quants, forced_qties):
        ...
        description = dict((product.id, '') for product in all_products)
        ...
        for mapping, qty in qtys_grouped.items():
            ...
            val_dict = {
                'picking_id': self.id,
                'description': mapping.product.name_get()[0][1],
